<?php 
    ...
    if( isset($_POST["cari"]) ){
        ...
        $query = "SELECT * FROM tiket_kereta WHERE 
                dari='$dari' AND
                tujuan='$tujuan' AND
                berangkat='$berangkat'";
    ...
    if( isset($_POST["pilihpulang"]) ){
        ...
        if( !isset($_SESSION["tmp"]) ){
            header('Location: tiket.php?jenis=keretaapi');
        }
    ...
    if( isset($_POST["end"]) ){
        ...
        $checkout1 = fetchAssoc(query("SELECT * FROM tiket_kereta WHERE id='$id_tiket_berangkat'"), SINGLE);
        $checkout2 = fetchAssoc(query("SELECT * FROM tiket_kereta WHERE id='$id_tiket_pulang'"), SINGLE);
