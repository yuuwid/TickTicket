...
$('#usersModal').on('show.bs.modal', function (event) {
    ...
    var data = button.data('dataid')
    var name = button.data('nameinput')
    var modal = $(this)

    if (name == 'hapus_id'){
        modal.find('.modal-title').html("Apakah anda ingin menghapus akun ini ?")
    } else if (name == 'bann_id'){
        modal.find('.modal-title').html("Apakah anda ingin menonaktifkan akun ini ?")
    }

    modal.find('.modal-body #id_user').val(data)    
    modal.find('.modal-body #id_user').attr('name', name)
})